...
if __name__=="__main__":
    ...
    np.savetxt("./regret_total/TotalRegrets_n%d_T_%d" % (n, T), totalregretVec, fmt='%.10f')
    ...
    np.savetxt("./regret_ratio/RegretRatio_Pure_n%d_T_%d" % (n, T), regretRatio, fmt='%.10f')
    
    
    os.makedirs('./regretvec/',exist_ok=True)
    np.savetxt("./regretvec/RegretVec_Pure_n%d_T_%d" % (n, T), regretVec, fmt='%.10f')
    
    os.makedirs('./posted_price/',exist_ok=True)
    np.savetxt("./posted_price/postedprice_Pure_n%d_T_%d" % (n, T), postedprice, fmt='%.10f')
