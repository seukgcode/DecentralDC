...
与normal区别：
1、生成可以访问当前db的参与方列表时，限制可访问方数量为5
2、生成多方计算行为范式时，多方计算参与方设置上限为5
...
for i in range(ParticipantNum):
    for db in DB_share_name_list[i]:
        ...
        permit_list = random.sample(candidates, random.randint(1, 5))  # 生成可以访问当前db的参与方列表
        bx_share.append(sorted(permit_list))
        Bx_paradigm_info.append(bx_share)

# 生成多方计算行为范式
Bx_multiCompute_num = round(Bx_share_num * 0.4)  # 定义多方计算行为的数量
for i in range(Bx_multiCompute_num):
    ...
    members = []  # 多方计算参与方
    members_index = sorted(random.sample(range(0, ParticipantNum), random.randint(2, 5)))
