...
def Read_Query_feature_vector(T, n):
    ...
    fX = open("./new_query/X_T_%d_n_%d"%(T,n))
    for t in range(T):
        line = fX.readline()
        if not line:
            break
...
def Read_Query_market_value_thetastar(T, n):
    ...
    fTheta = open("./new_query/theta_T_%d_n_%d"%(T,n))
...
if __name__=="__main__":
    ...
    os.makedirs('./regret_total/',exist_ok=True)
    np.savetxt("./regret_total/TotalRegrets_RP_n%d_T_%d" % (n, T), totalregretVec, fmt='%.10f')
    ...
    os.makedirs('./regret_ratio/',exist_ok=True)
    np.savetxt("./regret_ratio/RegretRatio_RP_n%d_T_%d" % (n, T), regretRatio, fmt='%.10f')
    
    
    
    os.makedirs('./regretvec/',exist_ok=True)
    np.savetxt("./regretvec/RegretVec_RP_n%d_T_%d" % (n, T), regretVec, fmt='%.10f')
    
    os.makedirs('./posted_price/',exist_ok=True)
    np.savetxt("./posted_price/postedprice_RP_n%d_T_%d" % (n, T), postedprice, fmt='%.10f')
    
    os.makedirs('./reserve_price/',exist_ok=True)
    np.savetxt("./reserve_price/reserveprice_RP_n%d_T_%d" % (n, T), reserveprice, fmt='%.10f')
