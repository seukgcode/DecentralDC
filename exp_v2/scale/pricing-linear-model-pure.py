...
if __name__=="__main__":
    ...
    for FILE_DIR in dir_list:
        ...
        np.savetxt(FILE_DIR + 'regret_total/TotalRegrets_n%d_T_%d' % (n, T), totalregretVec, fmt='%.10f')
        ...
        np.savetxt(FILE_DIR + 'regret_ratio/RegretRatio_Pure_n%d_T_%d' % (n, T), regretRatio, fmt='%.10f')
        
        
        os.makedirs(FILE_DIR + 'regretvec/',exist_ok=True)
        np.savetxt(FILE_DIR + 'regretvec/RegretVec_Pure_n%d_T_%d' % (n, T), regretVec, fmt='%.10f')
        
        os.makedirs(FILE_DIR + 'posted_price/',exist_ok=True)
        np.savetxt(FILE_DIR + 'posted_price/postedprice_Pure_n%d_T_%d' % (n, T), postedprice, fmt='%.10f')
